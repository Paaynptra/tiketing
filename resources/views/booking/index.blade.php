    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('home') }}"
                   class="inline-flex items-center px-3 py-2 rounded-md border border-warm-300 text-warm-800 bg-white hover:bg-warm-50 focus:outline-none focus:ring-2 focus:ring-brand-accent-500">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0h6"/>
                    </svg>
                    Home
                </a>
                <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('packages') }}"
                   class="inline-flex items-center px-3 py-2 rounded-md border border-warm-300 text-warm-800 bg-white hover:bg-warm-50 focus:outline-none focus:ring-2 focus:ring-brand-accent-500">
        ...
        <div class="bg-white ring-1 ring-green-400 shadow-lg sm:rounded-lg p-8 relative overflow-hidden">
            ...
            <form method="POST" action="{{ route('booking.store') }}" class="space-y-6">
                ...
                <div class="flex items-center justify-between pt-4 border-t border-green-400">
                    <a href="{{ route('booking.history') }}" class="text-green-700 hover:text-green-900 underline">Riwayat Pemesanan</a>
                    <button type="submit"
